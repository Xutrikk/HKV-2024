...
namespace Semantic {
	bool semanticsCheck(Lex::Tables& tables, Log::LOG& log) {
		...
		for (int i = 0; i < tables.lextable.size; i++)
		{
			...
			case LEX_EXTRA: {
				if (tables.lextable.table[i + 1].lexema != LEX_TYPE && tables.lextable.table[i + 1].lexema != LEX_FUNCTION) {
					sem_ok = false;
					throw ERROR_THROW_IN(312, tables.lextable.table[i].sn, -1);
				}
				break;
			}
			case LEX_TYPE: {
			...
			case LEX_ID: {
				if (tables.lextable.table[i - 1].lexema == LEX_FUNCTION && tables.lextable.table[i - 3].lexema != LEX_EXTRA) {
			...
			case LEX_EQUAL: {
				...
				if (strcmp(tables.idtable.table[tables.lextable.table[i - 1].idxTI].scope, tables.idtable.table[tables.lextable.table[k].idxTI].id) == 0 && tables.idtable.table[tables.lextable.table[k].idxTI].idtype == IT::F)
					throw ERROR_THROW_IN(320, tables.lextable.table[j].sn, i);
