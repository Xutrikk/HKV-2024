...
#define	TS(n) Rule::Chain::T(n)
	Greibach greibach(NS('S'), TS('$'),
		15,
		Rule(NS('Z'), GRB_ERROR_SERIES + 1,			//неверна€ структура функции 1
			2,
			Rule::Chain(5, TS('f'), TS('i'), TS('('), NS('G'), TS(')')),
			Rule::Chain(4, TS('f'), TS('i'), TS('('), TS(')'))
		),
		Rule(NS('S'), GRB_ERROR_SERIES + 0,			//неверна€ структура программы 1
			5,
		...
		Rule(NS('D'), GRB_ERROR_SERIES + 7,			//инициализаци€ переменной 1
			1,
			Rule::Chain(2, TS('t'), NS('W'))
		),
		Rule(NS('W'), GRB_ERROR_SERIES + 8,			//присвоение значени€ 1
			5,
		...
		Rule(NS('M'), GRB_ERROR_SERIES + 10,			//ожидаетс€ идентификатор или литерал 1
			2,
			Rule::Chain(1, TS('l')),
			Rule::Chain(1, TS('i'))
		...
		Rule(NS('K'), GRB_ERROR_SERIES + 11,//ошибка в структуре условного выражени€ 1
			3,
			Rule::Chain(5, TS(':'), NS('E'), TS('$'), NS('A'), TS('$')),
			Rule::Chain(4, TS(':'), NS('E'), TS('$'), TS('$')),
			Rule::Chain(5, TS(':'), NS('E'), TS('$'), TS('$'), NS('N'))
		),
		Rule(NS('A'), GRB_ERROR_SERIES + 12,			//ќшибка построени€ условного выражени€ 1
			3,
			Rule::Chain(3, TS('c'), TS(':'), NS('Y')),
			Rule::Chain(3, TS('w'), TS(':'), NS('Y')),
			Rule::Chain(2, TS('v'), NS('Y'))
		),
		Rule(NS('Y'), GRB_ERROR_SERIES + 13,	//ошибка тела выражени€/цикла 1
			2,
			Rule::Chain(4, TS('['), NS('N'), TS(']'), NS('A')),
			Rule::Chain(3, TS('['), NS('N'), TS(']'))
		),

		Rule(NS('N'), GRB_ERROR_SERIES + 14,		//ошибка в объ€влении переменной 1
			12,
