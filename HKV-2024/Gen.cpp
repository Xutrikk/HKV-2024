...
static int countOfLogicOperations = 0;

void header(Lex::Tables& tables, vector<string>& code)
{
    code.push_back(BEGIN);
    code.push_back(EXTRN);
    ...
    for (int i = 0; i < tables.idtable.size; i++)
    {
        ...
        if (tables.idtable.table[i].idtype == IT::IDTYPE::V)
        {
            ...
            switch (temp.iddatatype)
            {
            case IT::IDDATATYPE::INT: str = str + " sdword 0";
                break; //Если целочисл
            case IT::IDDATATYPE::STR: str = str + " dword ?";
                break; //Если строка
            case IT::IDDATATYPE::CHAR: str = str + " dword ?";
                break; //Если символ
            case IT::IDDATATYPE::BOOL: str = str + " word ?";
                break;
            }
            vData.push_back(str);
        }
    }
    code.insert(code.end(), vConst.begin(), vConst.end());
    code.insert(code.end(), vData.begin(), vData.end());
    code.push_back(CODE);
}

string ExtraFunc(Lex::Tables& tables, int i, string name, int param)
{
...
string headerOfFunc(Lex::Tables& tables, int i, string name, int param)
{
    ...
    while (number > 0)
    {
        temp = temp + string(IT_ENTRY(j).scope) + string(IT_ENTRY(j).id) + (IT_ENTRY(j).iddatatype == IT::INT
            ? " : sdword, "
            : " : dword, ");
        number--;
        j += 3;
...
string genStateCode(Lex::Tables& tables, int i, string& cyclecode)
{
    ...
    if (!inCycle)
    {
        ...
        for (int j = i + 6; LEXEMA(j) != LEX_DOL; j++)
            //для того чтобы начать с первого символа в $ $ state:n>m$correctly:[]wrong[]$
        {
        ...
        {
            auto scope = left.idtype == IT::L ? "" : left.scope;
            str = str + "push " + scope + left.id + "\n";
            str = str + "pop bx\n";
            b = true;
        }
        ...
        case LEX_OR:
        {
            auto scope = right.idtype == IT::L ? "" : right.scope;
            str = str + "or bx," + scope + right.id + "\n\n";
            str = str + "cmp bx, TRUE\n";
            correctlyStroke = "jz";
            WrongStroke = "jnz";
            break;
...
string generateEqual(Lex::Tables& tables, int pos)
{
    ...
    case IT::INT:
    {
        ...
        for (int j = pos + 1; LEXEMA(j) != LEX_SEMICOLON; j++) //начиная с первого правого операнда
        {
            ...
            case '@':
            {
                ...
                for (size_t i = 0; i < entrOfFunc.numberOfParam; i++)
                {
                    str.pop_back();
                ...
                for (size_t i = 0; i < entrOfFunc.numberOfParam; i++)
                {
                    ...
                    if (entr.idtype == IT::P || entr.idtype == IT::V)
                    {
                        if (entr.iddatatype == IT::CHAR || entr.iddatatype == IT::STR)
                            str = str + "push offset " + entr.scope + entr.id + "\n";
                        if (entr.iddatatype == IT::INT)
                            str = str + "push " + entr.scope + entr.id + "\n";
                    }
                    else if (entr.idtype == IT::L) {
                        if (entr.iddatatype == IT::CHAR || entr.iddatatype == IT::STR)
                            str = str + "push offset " + entr.id + "\n";
                        if (entr.iddatatype == IT::INT)
                            str = str + "push " + entr.id + "\n";
                    }
                    positionOfFunc--;
                }
    ...
    case IT::STR:
    {
        ...
        if (LEXEMA(pos + 1) == LEX_ID && e1.idtype == IT::F)
        {
            //вызов функции
            str = str + callFunc(tables, pos + 1);
            str = str + "mov " + e.scope + e.id + ", eax";
        }
        else if (LEXEMA(pos + 1) == LEX_LITERAL)
        {
            //литерал
...
namespace Generator
{
    ...
    {
        ofstream asmFile("C:\\BSTU\\SEM3\\HKVKPO2024\\HKV-2024\\Generated\\Generated.asm");
        vector<string> v;
        ...
        for (int i = 0; i < tables.lextable.size; i++)
        {
            ...
            case LEX_EXTRA:
                buffForFuncName = IT_ENTRY(i + 3).id;
                numberOfParam = tables.idtable.table[tables.lextable.table[i + 3].idxTI].numberOfParam;
                buff = ExtraFunc(tables, i + 3, buffForFuncName, numberOfParam);
                break;
            case LEX_FUNCTION:
            {
                if (LEXEMA(i - 2) == LEX_EXTRA)
                {
                    break;
                }
            ...
            case LEX_WRONG:
            {
                ...
                {
                    //проверка на то является ли стейт вложенным
                    nesting++;
                }
            ...
            case LEX_DOL:
            {
                if (LEXEMA(i - 1) == LEX_BRACELET)
                {
                    ...
                    {
                        //если стейт вышел из вложенности
